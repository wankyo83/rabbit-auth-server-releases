# Rabbit 인증 서버 0.3.4 — Docker/NAS 설치 방법
...
`docker-compose.yml`만 따로 옮기면 빌드에 필요한 소스를 찾을 수 없어 설치되지 않습니다.
...
3. 프로젝트를 빌드하고 시작합니다.
4. 최초 설치는 기본 이미지 다운로드와 빌드 때문에 시간이 걸릴 수 있습니다.
...
예전 서버 이미지가 실행 중일 수 있습니다. 0.3.4 설치 파일로 다시 빌드·재생성하고 다음 설정이 적용됐는지 확인합니다.
...
- 새 설치 파일로 소스를 교체한 뒤 프로젝트를 다시 빌드·재생성합니다.
