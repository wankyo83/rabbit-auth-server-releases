    ...
    while (!job.cancelled && this.now()<until) {
      ...
      if (!complete(rows,expected)) {
        ...
        if (dom?.notice === 'source_upload_pending') {
          ...
          return {kind:'images',expected:1,referer:job.origin+'/',userAgent:await page.evaluate(() => navigator.userAgent),
      ...
      if (complete(rows,expected)) {
        ...
        await this.saveState(job);
        return {kind:'images',expected,referer:job.origin+'/',userAgent,pages:rows.sort((a,b)=>a.page-b.page)};
      }
