# Rabbit 인증 서버 0.3.10 — Docker/NAS 설치 방법
...
Newtoki 유료 잠금 회차는 무한 대기하지 않고 안내 이미지 1장으로 표시합니다. 무료 전환 시간은 관리 화면의 WebView에서 확인하세요.
...
예전 서버 이미지가 실행 중일 수 있습니다. 0.3.10의 `docker-compose.yml`로 프로젝트를 다시 빌드·재생성하고 다음 설정이 적용됐는지 확인합니다.
...
- 특정 버전으로 되돌릴 때는 이미지 태그 `stable`을 원하는 버전(예: `0.3.10`)으로 바꾸고 다시 `pull`과 `up -d`를 실행합니다.
