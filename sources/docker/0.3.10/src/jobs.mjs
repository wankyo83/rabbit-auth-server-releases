...
export function cookieHeaderForUrl(cookies, rawUrl, nowSeconds=Math.floor(Date.now()/1000)) {
  const url=new URL(rawUrl), host=url.hostname.toLowerCase(), requestPath=url.pathname || '/';
  const parts=(Array.isArray(cookies)?cookies:[]).filter(cookie => {
    const domain=String(cookie.domain || '').replace(/^\./,'').toLowerCase();
    const domainMatches=String(cookie.domain || '').startsWith('.')
    ...
    return domainMatches && pathMatches && (!cookie.secure || url.protocol==='https:') &&
      (expires<=0 || expires>nowSeconds) && /^[^\x00-\x20\x7f()<>@,;:\\"/\[\]?={}]+$/.test(String(cookie.name || '')) &&
      !/[\r\n]/.test(String(cookie.value || ''));
  }).sort((a,b)=>String(b.path || '/').length-String(a.path || '/').length);
  const selected=[];
  for (const cookie of parts) {
    const next=String(cookie.name)+'='+String(cookie.value || '');
    if (selected.join('; ').length+next.length+(selected.length?2:0)>16384) break;
    selected.push(next);
  }
  return selected.join('; ');
}

async function attachImageCookies(context, rows) {
  const browserCookies=await context.cookies().catch(()=>[]);
  return rows.map(row => ({...row,cookies:row.urls.map(url=>cookieHeaderForUrl(browserCookies,url))}));
}
  ...
  async collect(job) {
    ...
    while (!job.cancelled && this.now()<until) {
      ...
      if (!complete(rows,expected)) {
        ...
        if (['source_upload_pending','paid_content_locked'].includes(dom?.notice)) {
      ...
      if (complete(rows,expected)) {
        ...
        const pages=await attachImageCookies(context,rows.sort((a,b)=>a.page-b.page));
        await this.saveState(job);
        return {kind:'images',expected,referer:refererFor(job),userAgent,pages};
      }
