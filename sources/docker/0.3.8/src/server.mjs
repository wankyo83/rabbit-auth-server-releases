...
export function createServer({ password, authMode = 'password', jobs, novncDir = '/usr/share/novnc',
  ...
  publicDir = path.join(root, 'public'), viewer = null, version = '0.3.8' }) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {initialization=jobs.init();await initialization;console.log('Rabbit Auth Server 0.3.8 ready (URL-only, concurrency '+jobs.concurrency+').');}
