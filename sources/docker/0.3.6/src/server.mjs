...
export function createServer({ password, authMode = 'password', jobs, novncDir = '/usr/share/novnc',
  ...
  publicDir = path.join(root, 'public'), viewer = null, version = '0.3.6' }) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === 'GET' && req.url?.split('?')[0] === '/assets/source-upload-pending.png')
      return await staticFile(res, publicDir, 'source-upload-pending.png');
    if (!canAccess(req)) {
      ...
      if (match) {
        ...
        if (req.method === 'POST') {
          ...
          if (action === 'manifest') {
            ...
            if (manifest.notice === 'source_upload_pending') {
              ...
              manifest.pages=[{page:1,urls:[sourceUploadPendingImageUrl]}];
            }
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {initialization=jobs.init();await initialization;console.log('Rabbit Auth Server 0.3.6 ready (URL-only, concurrency '+jobs.concurrency+').');}
