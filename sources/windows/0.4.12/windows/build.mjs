...
let buildKit=source;
while(path.basename(buildKit).toLowerCase()!=='build-kit' && path.dirname(buildKit)!==buildKit)buildKit=path.dirname(buildKit);
if(path.basename(buildKit).toLowerCase()!=='build-kit')throw Error('Source must be inside workspace/build-kit');
const workspace=path.dirname(buildKit);
...
await walk(target);await fs.writeFile(path.join(target,'checksums.json'),JSON.stringify({version:'0.4.12-windows',files:hashes},null,2));
