...
const server=createServer({password:process.env.SERVER_KEY,jobs,allowedHosts:new Set(['*']),
  publicDir:fileURLToPath(publicDir),viewer:createViewer(jobs,publicDir),version:'0.4.10-windows'});
...
try {
  ...
  if(!stopping){await status('ready');await log('Ready; protocol=1; URL-only; concurrency=2');}
} catch(e) {
