import test from 'node:test';import assert from 'node:assert/strict';import http from 'node:http';import crypto from 'node:crypto';import fs from 'node:fs/promises';
...
test('Windows form keeps the full page and address list independently scrollable',async()=>{
  const source=await fs.readFile(new URL('../windows/ServerForm.Designer.cs',import.meta.url),'utf8');
  assert.match(source,/scrollHost=new Panel\{[^\n]*AutoScroll=true/);
  assert.match(source,/addressScroll=new Panel\{[^\n]*AutoScroll=true/);
  assert.match(source,/AutoSize=true,AutoSizeMode=AutoSizeMode\.GrowAndShrink/);
  assert.match(source,/layout\.MinimumSize=new Size\(0,Math\.Max\(870,scrollHost\.ClientSize\.Height\)\)/);
  assert.match(source,/new RowStyle\(SizeType\.Absolute,230\)/);
  assert.match(source,/MinimumSize=new Size\(720,480\)/);
  assert.doesNotMatch(source,/MinimumSize=new Size\(820,860\)/);
});
